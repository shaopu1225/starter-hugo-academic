...
在前边的分析中，我们也看到了，矩阵吸收仅能降低带来的多余的计算复杂度而无法完全避免，在训练和Prefill场景中，多出的计算复杂度变成了平方级，即$O(N^2(d_c-d_h))$，这就不太划算了。所以在训练和prefill阶段，只会做低秩压缩，而不会做矩阵吸收。
...
这破坏了矩阵吸收的基本形式，每次在计算位置$i$的注意力分数时，还需要使用位置$j$的旋转矩阵，其不能被吸收到固定权重里，也就无从谈起提升推理效率。
